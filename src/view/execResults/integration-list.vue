<template>
    ...
    <el-descriptions :column="2" :size="size" class="descriptBox" direction="horizontal" :style="blockMargin">
      ...
      <el-descriptions-item label="执行进度" :span="2">
        ...
        <div style="padding-right: 40px">
          {{ planData.pass_num + planData.failed_num }} /
          {{ planData.pass_num + planData.failed_num + planData.un_run_num }}
        </div>
        <el-progress
          :percentage="
            Math.round(
              ((planData.pass_num + planData.failed_num) * 100) /
                (planData.pass_num + planData.failed_num + planData.un_run_num),
            )
          "
          style="width: 400px"
  ...
  <div>
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column :show-overflow-tooltip="true" fixed prop="scene_id" label="用例编号"> </el-table-column>
      <el-table-column fixed prop="case_title" label="用例名称" :show-overflow-tooltip="true"> </el-table-column>
      <!-- <el-table-column
      ...
      <el-table-column prop="exe_env" label="环境" :show-overflow-tooltip="true"> </el-table-column>
...
export default {
  ...
  setup(props, context) {
    ...
    // 筛选按钮（全部，成功，失败，未执行）未执行不可点击
    const selectType = reactive({
      ...router.currentRoute.value.query,
    })
    const stateList = ref([])
    let setStartList = function () {
      stateList.value = [
        {
          label: '全部',
          value: props.planData.pass_num + props.planData.failed_num + props.planData.un_run_num,
          type: '',
        },
        { label: '成功', value: props.planData.pass_num ? props.planData.pass_num : 0, type: 'success' },
        { label: '失败', value: props.planData.failed_num ? props.planData.failed_num : 0, type: 'danger' },
        { label: '未执行', value: props.planData.un_run_num ? props.planData.un_run_num : 0, type: 'info' },
      ]
    }
    setStartList()
    watch(
    ...
    const selectState = label => {
      if (label === '成功') {
        selectType.exe_result = 'pass'
      } else if (label === '失败') {
        selectType.exe_result = 'failed'
      } else {
