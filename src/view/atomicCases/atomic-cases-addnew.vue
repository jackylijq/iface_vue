        ...
        <el-tab-pane label="全量配置" name="0" ref="sectionRef">
          ...
          >
            ...
            <el-table-column :show-overflow-tooltip="true" prop="queryValue" label="参数化值">
              <template v-slot="scope">
                <el-input v-if="scope.row.checked" v-model="scope.row.queryValue" size="small"></el-input>
              </template>
            </el-table-column>
          </el-table> -->
          <div class="headerTitle">
            响应参数
            <atomicCasesResponseWord :formData="responseWord" :backTableData="backTableData" />
          </div>
...
<script>
import { ref, reactive, unref, onMounted, computed, watchEffect, watch } from 'vue'
  ...
  "check_position":""
    ...
    const handleClick = (tab, event) => {
      ...
      if (data.activeName == '1') {
        ...
        new JsonToTableData().postMessage([JSON.parse(JSON.stringify(requestJson.value))]).then(res => {
          ...
          // console.log(queryTableData.value, 'queryTableData.value')
          addBaseInfo(queryTableData.value, 'query')
          addBaseInfo(backTableData.value, 'response')
    ...
    function childrenCheck(val) {
      val.forEach((item, index) => {
        if (item.checked) {
          ...
          item.queryValue = `\${${item.name}}` //'$'+ item.name
        } else {
    ...
    let getIfaceDetail = async function (iface_id) {
      ...
      if (typeof req_headers == 'string' && req_headers !== '') {
        ...
        req_headersArr = new Function(`return [${req_headersJson}]`)()
      } else if (req_headers) {
        req_headersArr = req_headers
      }
      let headerBase = [
      ...
      if (typeof req_body == 'string' && req_body !== '') {
        if (req_body.indexOf('ObjectId') !== -1) {
          ...
          let queryJson = new Function(`return [${req_body}]`)()
          if (queryJson.items) {
      ...
      if (name) {
        responseWord.value.code_name = name
        responseJson.value.res_code = value
      }
