<template>
    <el-table class="headerTable" :data="tableData" :default-expand-all="true" stripe row-key="name"
        ...
        <el-table-column :show-overflow-tooltip="true" prop="value" label="参数值">
            <template v-slot="scope">
                <el-input disabled v-model="scope.row.value" size="small"></el-input>
            </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" prop="value" label="是否参数化">
            <template v-slot="scope">
                <el-checkbox disabled v-model="scope.row.checked" @change="queryCheck">{{ scope.row.checked ? '是' : '否'
                }}</el-checkbox>
            </template>
        </el-table-column>
