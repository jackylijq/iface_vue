        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="等待时间" prop="wait_time">
              <el-input-number
                v-model="formData.wait_time"
                controls-position="right"
                :min="0"
                style="width: calc(100% - 20px); height: 32px"
              />
              <span class="tip">
                <el-popover
                  placement="right"
        ...
        <el-table-column :show-overflow-tooltip="true" prop="" label="">
          <template v-slot="scope">
            <i
              ...
              style="font-size: 20px; margin: 0 18px; cursor: pointer"
              @click="addHeaderInfo(scope.row)"
            ></i>
            <i
              class="el-icon-delete"
              style="font-size: 20px; cursor: pointer"
              @click="deleteHeaderInfo(scope.row)"
              v-show="
                scope.row.name !== 'Content-Type' && scope.row.name !== 'username' && scope.row.name !== 'password'
              "
            ></i>
          </template>
        </el-table-column>
        ...
        <el-table-column :show-overflow-tooltip="true" prop="" label="">
          <template v-slot="scope">
            <i
              class="el-icon-view"
              style="font-size: 20px; margin: 0 18px; cursor: pointer"
              @click="previewHeaderInfo(scope)"
            ></i>
            <i
              class="el-icon-delete"
              style="font-size: 20px; cursor: pointer"
              @click="deleteFunInfo(scope)"
              v-show="
                scope.row.name !== 'Content-Type' && scope.row.name !== 'username' && scope.row.name !== 'password'
              "
            ></i>
    ...
    <div class="stepTwo" v-show="step == '2'">
      <el-tabs v-model:activeName="data.activeName" @tab-click="handleClick" class="table-box">
        <el-tab-pane label="全量配置" name="0" ref="sectionRef">
          <div class="headerTitle">请求参数 <atomicCasesParamType :formData="formData" /></div>

          <!-- 调整成虚拟表格 -->
          <RequestTable :data="queryTableData" :expandedRowKeys="expandedKey" />
          ...
          >
            ...
            <el-table-column :show-overflow-tooltip="true" prop="value" label="是否参数化">
              <template v-slot="scope">
                <el-checkbox v-model="scope.row.checked" @change="queryCheck">{{
          ...
          <div class="headerTitle">
            响应参数 <atomicCasesResponseWord :formData="responseWord" :backTableData="backTableData" />
          </div>
          <el-table
            :data="backTableData"
            :default-expand-all="true"
          ...
          >
            ...
            <el-table-column :show-overflow-tooltip="true" prop="value" label="是否参数化">
              <template v-slot="scope">
                <el-checkbox v-model="scope.row.checked" @change="responseCheck()">{{
                  scope.row.checked ? '是' : '否'
                }}</el-checkbox>
              </template>
            </el-table-column>
            <el-table-column :show-overflow-tooltip="true" prop="resonseName" label="参数化名称">
              <template v-slot="scope">
                <el-input v-model="scope.row.resonseName" :disabled="!scope.row.checked">{{
                  scope.row.checked ? scope.row.resonseName : ''
                }}</el-input>
              </template>
            </el-table-column>
    ...
    <div class="stepThree" v-show="step == '3'">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item
              label="等待时间："
              prop="wait_time"
              :rules="{ pattern: /^[0-9]\d*$/, message: '请输入大于或等于0的整数！', trigger: 'change' }"
            >
              <template slot="label">
                <div class="label">等待时间：</div>
              </template>
              <el-input-number
                v-model="checkForm.wait_time"
                controls-position="right"
                :min="0"
                style="width: 150px; height: 32px"
              />
              <span class="timeTip">
                <el-popover placement="right" title="Note" :width="220" trigger="hover" content="执行检查的等待时间">
                  <template #reference>
                    <i class="el-icon-question"></i>
                  </template>
                </el-popover>
      ...
      <el-tabs v-model="checkTabName" @tab-click="handleClickTab">
          ...
          <keep-alive>
            <div>
              ...
              <div class="sqlCheck">
                ...
                <el-input type="textarea" :rows="3" v-model="checkSql.response" style="width: 650px"> </el-input>
                <span class="tipTwo">
                  <el-popover
                    placement="right"
                    title="Note"
              ...
              >
                ...
                <el-table-column :show-overflow-tooltip="true" prop="checked" label="是否检查" width="auto">
                  <template v-slot="scope">
                    <el-checkbox
                      v-if="scope.row.name !== 'code'"
                      v-model="scope.row.isChecked"
                      @change="responseSqlCheck(scope.row)"
                      >{{ scope.row.isChecked ? '是' : '否' }}</el-checkbox
                    >
        ...
        <el-tab-pane label="请求数据检查">
          <keep-alive>
            <div>
              ...
              <div class="sqlCheck">
                <span class="checkTitle">检查sql：</span>
                <el-input type="textarea" :rows="3" v-model="checkSql.request" style="width: 650px"> </el-input>
              </div>

              <RequestTableValidate
                :data="checkQueryTableData"
                :expandedRowKeys="expandedKey"
                :isCheckedFn="requestSQLCheck"
              />
              <!--               
              <el-table
                :data="checkQueryTableData"
              ...
              >
                ...
                <el-table-column :show-overflow-tooltip="true" prop="value" label="参数值"> </el-table-column>
                <el-table-column :show-overflow-tooltip="true" prop="value" label="是否检查">
                  <template v-slot="scope">
                    <el-checkbox v-model="scope.row.isChecked" @change="requestSQLCheck(scope.row)">{{
        ...
        <el-tab-pane label="函数检查">
          <keep-alive>
            <div>
              <el-alert title="使用底层函数对接口的响应数据进行检查" type="info" class="tipInfo" show-icon> </el-alert>
              ...
              <el-table :data="checkFuncTableData" stripe class="headerTable">
                ...
                <el-table-column :show-overflow-tooltip="true" prop="edit_uid" label="创建人"> </el-table-column>
                ...
                <el-row>
                  ...
                  <el-col :span="6">
                    <el-form-item label="函数英文名称：">
                      <el-input style="width: 80%" v-model="searchForm.en_name"> </el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item label="函数中文名称：">
                      <el-input style="width: 80%" v-model="searchForm.cn_name"> </el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-button type="primary" @click="searchList"
                      ><i class="el-icon-search" style="margin-right: 3px"></i>查询</el-button
                    >
                    <el-button @click="refreshSearch"
                      ><i class="el-icon-refresh-right" style="margin-right: 3px"></i>重置</el-button
                    >
                  </el-col>
                </el-row>
              </el-form>
...
<script>
import { ref, reactive, unref, onMounted, computed, watch, watchEffect } from 'vue'
...
import atomicCasesParamType from './atomic-cases-param-type.vue'
import atomicCasesResponseWord from './atomic-cases-response-word.vue'
import RequestTable from './requestTable.vue'
import RequestTableValidate from './requestTableValidate.vue'
...
export default {
  components: {
    ...
    JsonEditorVue,
    AtomicCasesFun,
    atomicCasesParamType,
    atomicCasesResponseWord,
  },
  setup() {
  ...
  "check_position":""
    ...
    let tableParams = JSON.parse(JSON.stringify(router.query))
    ...
    watchEffect(() => {
      if (unref(responseWord).code_name) {
        backTableData.value.find(v => v.name === unref(responseWord).code_name).value = unref(responseWord).res_code
      }
    })
    ...
    //     if (oldV.code_name) {
    //       backTableData.value.find(v => v.name === oldV.code_name).value = ''
    //     }
    //     if (newV.code_name) {
    //       backTableData.value.find(v => v.name === newV.code_name).value = newV.res_code
    //     }
    //   },
    //   {
    //     deep: true,
    ...
    const computedShowCode = computed(() => {
      return unref(backTableData).find(v => v.name === unref(responseWord).code_name)
    })

    let expandedKey = ref([])
    function addQueryTableDataId(list, pid = '', keys = []) {
      list.forEach(ele => {
        ele.id = `${pid}-${ele.name}`
        if (ele.children) {
          keys.push(ele.id)
          addQueryTableDataId(ele.children, ele.id, keys)
        }
      })
    ...
    function getTableDataId(list, keys = []) {
      list.forEach(ele => {
        if (ele.children) {
          keys.push(ele.id)
          getTableDataId(ele.children, keys)
        }
      })
      return keys
    }
    ...
    const changeStep = function (val, from) {
      if (val == 2) {
        ...
        formData.value.generic_variables = funcData.value.reduce(
          (a, b) => ({
            ...a,
        ...
        // }
        // 处理id
        expandedKey.value = addQueryTableDataId(queryTableData.value)
        ...
        if (unref(responseWord).code_name && [null, undefined, NaN, ''].includes(unref(responseWord).res_code)) {
          ElMessage({
            type: 'error',
            message: `响应码值不能为空`,
          })
        ...
        let error = queryTableData.value.filter(v => v.checked).some(v => !/^\$\{.+\}$/.test(v.queryValue))
        if (error) {
          ElMessage({
            type: 'error',
            message: `请求参数-参数化值必须是\${}格式`,
          })
          return
        }
        ...
        checkResponseTableData.value.forEach(i => {
          if (i.name == unref(responseWord).code_name) {
            i.isChecked = true
            i.checkValue = unref(responseWord).res_code
          }
        })
      }
      step.value = val
    ...
    const handleClick = (tab, event) => {
      ...
      if (data.activeName == '1') {
        ...
        new JsonToTableData().postMessage([JSON.parse(JSON.stringify(requestJson.value))]).then(res => {
          // console.log('🚀 ~ JsonToTableData.postMessage ~ res:', res)
          queryTableData.value = res //backTrans(requestJson.value, 'query')

          expandedKey.value = getTableDataId(queryTableData.value)
          ...
          let { name, value } = unref(backTableData).find(v => v.name === 'code') || {}
          if (name) {
            responseWord.value.code_name = name
            responseWord.value.res_code = value
          }
        })
      }
    }
    ...
    const firstTrans = function (val, type) {
      ...
      val.forEach(item => {
        ...
        if (!item.children || item.children.length == 0) {
          if (item.valueType == 'object') {
            ...
            if (item.value == undefined) {
              ...
              obj[item.name] = String(item.value).length <= 16 ? Number(item.value) : String(item.value)
            }
        ...
        } else if (item.valueType == 'array' && item.children && item.children[0]?.name !== '0') {
    ...
    let getIfaceDetail = async function (iface_id) {
      ...
      const { req_headers, req_body, res_body, req_body_type, iface_name, request_url, request_method } = res.data
      formData.value.name = iface_name
      formData.value.address = request_url
      formData.value.method = request_method
      formData.value.req_body_type = req_body_type === 'form' ? 'form-data' : req_body_type
      if (typeof req_headers === 'string') {
        let req_headersJson = req_headers
          .replace(RegExp('[(]', 'g'), '')
          .replace(RegExp('[)]', 'g'), '')
          .replace(RegExp('ObjectId', 'g'), '')
        applyTableBaseData = new Function(`return [${req_headersJson}]`)() //JSON.parse('['+req_headersJson+']')
      } else {
    ...
    const save = async function () {
      ...
      let result_check = {
        ...
        request: {},
        res_code: {
          code_name: unref(responseWord).code_name,
          code_value: '',
        },
      ...
      if (checkResponseTableData.value[0] == 'number') {
        result_check.res_code.code_value = Number(unref(responseWord).res_code)
      } else {
        result_check.res_code.code_value = unref(responseWord).res_code
      }
      let headerObj = {}
      ...
      // let changeValue = checkResponseTableData.value
      ...
      if (res.code == 200) {
        ...
        vueRouter.push({
          ...
          query: {
            project_id: tableParams.project_id,
            group_id: tableParams.group_id,
            iface_id: tableParams.iface_id,
          },
        })
        ...
        window.sessionStorage.setItem(
          'atomic-cases-list_status',
          JSON.stringify({ defaultExpandedKeys, currentNodeKey }),
        )
    ...
    const getEditData = async function () {
      ...
      responseWord.value.code_name = tableParams.result_check.res_code.code_name
      responseWord.value.res_code = tableParams.result_check.res_code.code_value
      ...
      formData.value.address = request_url
      formData.value.method = request_method
      formData.value.req_body_type = tableParams.req_body_type

      if (typeof res.data.req_headers === 'string') {
        let req_headersJson = res.data.req_headers
          .replace(RegExp('[(]', 'g'), '')
          .replace(RegExp('[)]', 'g'), '')
          .replace(RegExp('ObjectId', 'g'), '')
        headerBaseInfo.value = new Function(`return [${req_headersJson}]`)()
      } else {
        headerBaseInfo.value = res.data.req_headers
      }
      applyTableData.value.forEach(i => {
        let sameItem = headerBaseInfo.value.find(e => {
