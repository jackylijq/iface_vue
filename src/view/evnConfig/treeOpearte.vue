<template>
  ...
  <el-dialog :title="dialogTitle" :append-to-body="true" v-model="addDialogShow" v-if="addDialogShow" width="600px">
    <component
      :is="computedType[dialogCom]"
      :data="data"
      :level="node.level"
      :isEdit="isEdit"
      @cancel-fn="addDialogShow = false"
      @submit-fn="onSubmit"
    ></component>
  </el-dialog>
</template>
<script setup>
import { computed, defineProps, ref, defineEmits, unref } from 'vue'
...
const emits = defineEmits(['refresh-product', 'refresh-group', 'remove-group', 'remove-product'])

const computedAddIconShow = computed(() => {
  return props.data.id == '-1'
})

const computedEditIconShow = computed(() => {
  return props.data.id !== '-1'
})

const computedDeleteIconShow = computed(() => {
  return computedEditIconShow.value
})
...
const computedType = computed(() => {
  return {
    ...
    deleteFn() {
      return axios({
        method: 'post',
        url: '/iftest/condition/group/del',
        data: { id: props.data.otherData.id },
      }).then(res => {
        if (res.code === 200) {
          emits(`remove-group`)
        }
...
const handleAddClick = function () {
  isEdit.value = false
  dialogCom.value = 'comAdd'
  dialogTitle.value = '增加分组'
...
const onSubmit = function () {
  if (unref(isEdit)) {
    computedType.value?.editCb()
  } else {
    computedType.value?.addCb()
  }
...
const handleEditClick = function () {
  isEdit.value = true
  dialogCom.value = 'comAdd'
  dialogTitle.value = `编辑${props.data.label}`
  addDialogShow.value = true
}

const handleDeleteClick = function () {
  ElMessageBox.confirm(`确认删除${computedType.value.label}-${props.data.label}？`, '提示', {
    // confirmButtonText: '确认',
    callback: async action => {
      if (action === 'confirm') {
        let res = await computedType.value.deleteFn()
        ElMessage({
...
<style lang="scss">
.base-tree .el-tree-node__content:hover .tree-operate {
